.PHONY: stress-tickets check-load-test-token validate-k6-tickets-list-profile validate-k6-tickets-create-profile validate-k6-tickets-create-update-profile prepare-k6-tickets-list k6-tickets-list seed-performance-tickets prepare-k6-tickets-create k6-tickets-create prepare-k6-tickets-create-update k6-tickets-create-update
...
K6_TICKETS_CREATE_UPDATE_TEST_TYPE ?= tickets-create-update-$(K6_PROFILE)
K6_TICKETS_CREATE_UPDATE_TEST_ID ?= $(K6_TICKETS_CREATE_UPDATE_TEST_TYPE)-$(shell date -u +%Y%m%dT%H%M%SZ)
...
validate-k6-tickets-create-update-profile:
	@node -e 'const configs = require("./tests/performance/k6/tickets/create-update-configs.json"); const profile = process.argv[1]; if (!Object.prototype.hasOwnProperty.call(configs, profile)) { console.error("K6_PROFILE must be one of: " + Object.keys(configs).join(", ")); process.exit(1); }' "$(K6_PROFILE)"
...
k6-tickets-list: check-load-test-token validate-k6-tickets-list-profile ## Run the selected k6 ticket-list profile (K6_PROFILE=smoke|load|stress).
...
prepare-k6-tickets-create-update: ## Reset local data and start tools for ticket create-and-update k6 tests.
	$(MAKE) docker-reset
	$(MAKE) docker-up-tools

k6-tickets-create-update: check-load-test-token validate-k6-tickets-create-update-profile ## Run the selected k6 ticket create-and-update profile (K6_PROFILE=smoke|load|stress).
	docker compose --profile performance --profile tools run --rm -e K6_TEST_ID=$(K6_TICKETS_CREATE_UPDATE_TEST_ID) k6 run -e K6_PROFILE=$(K6_PROFILE) -e K6_TEST_ID=$(K6_TICKETS_CREATE_UPDATE_TEST_ID) -e LOAD_TEST_METRICS_TOKEN="$${LOAD_TEST_METRICS_TOKEN:-$$(sed -n 's/^LOAD_TEST_METRICS_TOKEN=//p' .env 2>/dev/null | tail -n 1)}" -o experimental-prometheus-rw --tag source=k6 --tag test_type=$(K6_TICKETS_CREATE_UPDATE_TEST_TYPE) --tag environment=local --tag testid=$(K6_TICKETS_CREATE_UPDATE_TEST_ID) /scripts/tickets/create-update.js
