...
.PHONY: help install generate-proto build build-api-gateway build-identity build-tickets build-orders build-payments build-expiration test test-api-gateway test-tickets test-orders test-payments test-expiration serve-api-gateway serve-identity serve-tickets serve-orders serve-payments serve-expiration stress-tickets check-load-test-token k6-tickets-list k6-tickets-list-seeded k6-tickets-list-comparison seed-performance-tickets replay-orders-dlq replay-tickets-dlq replay-payments-dlq replay-expiration-dlq docker-build docker-up docker-up-tools docker-down docker-reset docker-logs docker-ps
