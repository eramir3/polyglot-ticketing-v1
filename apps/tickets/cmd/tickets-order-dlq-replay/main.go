...
func main() {
	...
	defer cancel()
	ack, err := consumer.ReplayTicketsDeadLetter(ctx, js, sequence)
	if err != nil {
		slog.Error("Tickets DLQ replay failed", "sequence", sequence, "error", err)
		os.Exit(1)
	}
	fmt.Printf("replayed Tickets DLQ sequence %d as source stream sequence %d\n", sequence, ack.Sequence)
}
